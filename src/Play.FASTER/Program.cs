...
void PopulateStore(FasterKV<CacheKey, CacheValue> store)
{
...
void RandomReadWorkload(FasterKV<CacheKey, CacheValue> store, int max)
{
    // Start session with FASTER
    using var s = store.For(new CacheFunctions()).NewSession<CacheFunctions>();
...
void InteractiveReadWorkload(FasterKV<CacheKey, CacheValue> store)
{
    // Start session with FASTER
    using var s = store.For(new CacheFunctions()).NewSession<CacheFunctions>();
